...
for coord in TILE_COORDS:
    ...
    # N meets NW, NE
    n_set = tuple(sorted([coord, add_coord(coord, DIRECTIONS["NW"]), add_coord(coord, DIRECTIONS["NE"])]))
    # NE meets NE, E
    ne_set = tuple(sorted([coord, add_coord(coord, DIRECTIONS["NE"]), add_coord(coord, DIRECTIONS["E"])]))
    # SE meets E, SE
    se_set = tuple(sorted([coord, add_coord(coord, DIRECTIONS["E"]), add_coord(coord, DIRECTIONS["SE"])]))
    # S meets SE, SW
    s_set = tuple(sorted([coord, add_coord(coord, DIRECTIONS["SE"]), add_coord(coord, DIRECTIONS["SW"])]))
    # SW meets SW, W
    sw_set = tuple(sorted([coord, add_coord(coord, DIRECTIONS["SW"]), add_coord(coord, DIRECTIONS["W"])]))
    # NW meets W, NW
    nw_set = tuple(sorted([coord, add_coord(coord, DIRECTIONS["W"]), add_coord(coord, DIRECTIONS["NW"])]))
