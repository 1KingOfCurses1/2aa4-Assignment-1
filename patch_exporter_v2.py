...
with open(r"c:\University\Second Year Programming\2AA4\2aa4-Assignment-1\edge_output.txt", "r") as f:
...
for line in lines:
    if "EdgePair" in line:
        match = re.search(r'EdgePair\((\d+), (\d+)\), new int\[\]\{(\d+), (\d+)\}', line)
        if match:
            u, v, nu, nv = match.groups()
            map_lines.append(f'        EDGE_TRANSLATION.put("{u}_{v}", new int[]{{{nu}, {nv}}});')
...
java_cod = re.sub(r'(    private String baseMapPath;)', lambda m: dict_code + "\n" + m.group(1), java_cod)
