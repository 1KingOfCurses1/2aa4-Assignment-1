    ...
    public Action chooseAction(Game game, Player player) {
        ...
        for (Node n : board.getNodes()) {
            if (board.isValidSettlementPlacement(n, player)) {
                availableActions.add(new Action(round, player.getId(), "BUILD_SETTLEMENT " + n.getId(), ActionType.BUILD_SETTLEMENT));
            }
        }
        ...
        for (Node n : board.getNodes()) {
            if (board.isValidCityPlacement(n, player)) {
                availableActions.add(new Action(round, player.getId(), "BUILD_CITY " + n.getId(), ActionType.BUILD_CITY));
            }
        }
        ...
        for (Edge e : board.getEdges()) {
            if (board.isValidRoadPlacement(e, player)) {
                if (e.getNodes().size() == 2) {
                    ...
                    availableActions.add(new Action(round, player.getId(), "BUILD_ROAD " + n1 + " " + n2, ActionType.BUILD_ROAD));
                }
            }
        }
